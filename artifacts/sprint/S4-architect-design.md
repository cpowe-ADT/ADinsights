...
- **Template registry** (`lib/dashboardTemplates.ts`): extend each `DashboardTemplateDefinition` with an OPTIONAL `layout` field. Unknown / missing → current whole-page render path continues.
  ...
  type SlotConfig = {
    ...
    title?: string;
    dataBinding?: {
      // purely descriptive; actual data comes from store
      slice:
        | 'campaign'
        | 'creative'
        | 'budget'
...
- `components/ParishMap.tsx` (Leaflet wrapper) — don't refactor; add the KPI-picker UI _inside_ `ParishMapDetail.tsx` above the map container and pass `selectedMetric` via the store.
