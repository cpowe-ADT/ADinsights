...
Custom connectors for Microsoft Ads, LinkedIn, and TikTok live under `sources/microsoft_ads/`, `sources/linkedin_ads/`, and `sources/tiktok_ads/`. Export the required environment variables (documented below) before packaging them with `airbyte-ci` or when importing the specs via the Airbyte UI.
...
- `microsoft_ads_source.yaml` – Connection payload for the custom Microsoft Ads connector in `sources/microsoft_ads/`.
...
Optional connectors are disabled by default in dbt via feature variables until their staging models are promoted. Toggle the corresponding variable in the target profile only after the first Airbyte sync succeeds and the dbt follow-up PR lands.
...
| Microsoft Ads (custom) | `AIRBYTE_MICROSOFT_ADS_ACCOUNT_ID`, `AIRBYTE_MICROSOFT_ADS_DEVELOPER_TOKEN`, `AIRBYTE_MICROSOFT_ADS_ACCESS_TOKEN`, `AIRBYTE_MICROSOFT_ADS_START_DATE` (optional), `AIRBYTE_MICROSOFT_ADS_LOOKBACK_DAYS` (optional)                                                                                                                                                                                                   |
...
- **Microsoft Ads:** Use an approved developer token and OAuth access token with access to the account. The custom connector emits the normalized `microsoft_ads_performance` stream from ad performance report rows; production rollout still requires external account approval, report job polling validation, and dbt staging promotion.
...
6. Flip the corresponding dbt feature flags only after the first sync succeeds and staging/mart support has shipped.
