...
def _write_evidence_rollup_files(repo_root, recommendation):
    for row in recommendation["evidence_rollup"].values():
        path = repo_root / row["evidence_link"]
        path.parent.mkdir(parents=True, exist_ok=True)
        if path.suffix == ".json":
            _write_json(path, {"schema_version": "test_g12_evidence.v1", "status": "passed"})
        else:
...
def _valid_recommendation(recommendation="cancel_dashthis_recommended"):
    ...
    for index in range(12):
        ...
        evidence_rollup[goal_id] = {
            "status": "passed",
            "evidence_link": f"docs/project/evidence/dashthis-replacement/{goal_id.lower()}-proof.md",
            "reviewer_approval": "approved",
        }
    dashthis_action = "cancel_after_acceptance" if recommendation == "cancel_dashthis_recommended" else "keep_active"
    return {
        ...
        "decision": {
            ...
            "dashthis_cancellation_date": "2026-06-21" if recommendation == "cancel_dashthis_recommended" else "",
        },
        "evidence_rollup": evidence_rollup,
        "cancellation_scope": {
            "included_datasets": ["paid_meta_ads", "organic_facebook_page", "content_ops"],
        ...
        "reviewer_signoffs": {
            ...
            "hannah": "approved",
            "nina": "approved",
            "business_owner": "approved_cancel" if recommendation == "cancel_dashthis_recommended" else "approved_keep",
        },
        "decision_change_log": [
            {
...
def test_valid_g12_final_recommendation_passes(tmp_path, monkeypatch, capsys):
    ...
    exit_code, result = _run_validator(recommendation_path, status_path, window_path, capsys)
...
def test_template_pending_values_fail(tmp_path, capsys):
    recommendation_path = tmp_path / "recommendation.json"
    recommendation_path.write_text(TEMPLATE.read_text(encoding="utf-8"), encoding="utf-8")

    exit_code, result = _run_validator(recommendation_path, None, None, capsys)

    assert exit_code == 1
    assert result["valid"] is False
    assert any("status must be final_decision_recorded" in error for error in result["errors"])
    assert any("references.status_manifest_valid must be true" in error for error in result["errors"])
...
def test_status_manifest_must_have_g0_g11_passed(tmp_path, capsys):
    ...
    assert exit_code == 1
    assert result["valid"] is False
    assert any("Status manifest G5 must be passed before G12" in error for error in result["errors"])
...
def test_target_must_match_g11_window(tmp_path, capsys):
    ...
    assert exit_code == 1
    assert result["valid"] is False
    assert any("target.report_definition_id must match G11 window" in error for error in result["errors"])


def test_cancel_recommendation_requires_business_owner_and_cancel_action(tmp_path, capsys):
    recommendation = _valid_recommendation()
    ...
    assert exit_code == 1
    assert result["valid"] is False
    assert any("decision.business_owner_acceptance must be true" in error for error in result["errors"])
    assert any("decision.dashthis_action must be cancel_after_acceptance" in error for error in result["errors"])
    assert any("reviewer_signoffs.business_owner is required" in error for error in result["errors"])
...
def test_cancel_date_must_not_precede_effective_date(tmp_path, capsys):
    ...
    assert exit_code == 1
    assert result["valid"] is False
    assert "decision.dashthis_cancellation_date must be on or after decision.effective_date." in result["errors"]
...
def test_keep_recommendation_rejects_cancellation_date(tmp_path, capsys):
    ...
    assert exit_code == 1
    assert result["valid"] is False
    assert "decision.dashthis_cancellation_date must be empty when cancellation is not recommended." in result["errors"]
...
def test_unfinished_evidence_rollup_fails(tmp_path, capsys):
    ...
    assert exit_code == 1
    assert result["valid"] is False
    assert any("evidence_rollup.G10.status must be passed" in error for error in result["errors"])
    assert any("final_acceptance.adversarial_no_blocker must be true" in error for error in result["errors"])
...
def test_evidence_rollup_rejects_denied_or_pending_approval(tmp_path, capsys):
    ...
    assert exit_code == 1
    assert result["valid"] is False
    assert any("evidence_rollup.G2.reviewer_approval must be an approved" in error for error in result["errors"])
    assert any("evidence_rollup.G3.reviewer_approval must be an approved" in error for error in result["errors"])
...
def test_reviewer_signoffs_reject_denied_signoff(tmp_path, capsys):
    ...
    assert exit_code == 1
    assert result["valid"] is False
    assert any("reviewer_signoffs.mira must be an approved" in error for error in result["errors"])
    assert any("reviewer_signoffs.omar must be an approved" in error for error in result["errors"])
...
def test_keep_recommendation_requires_business_owner_acceptance_value(tmp_path, capsys):
    ...
    assert exit_code == 1
    assert result["valid"] is False
    assert any("reviewer_signoffs.business_owner must approve or accept the final recommendation" in error for error in result["errors"])
...
def test_evidence_rollup_link_must_exist_under_evidence_root(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr(cli, "REPO_ROOT", tmp_path)
    recommendation = _valid_recommendation()
    ...
    assert exit_code == 1
    assert result["valid"] is False
    assert any("evidence_rollup.G4.evidence_link must be under docs/project/evidence/dashthis-replacement" in error for error in result["errors"])


def test_evidence_rollup_link_sensitive_contents_are_rejected(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr(cli, "REPO_ROOT", tmp_path)
    recommendation = _valid_recommendation()
    _write_evidence_rollup_files(tmp_path, recommendation)
    ...
    assert exit_code == 1
    assert result["valid"] is False
    assert any("Sensitive or user-level pattern detected in evidence_rollup.G8.evidence_link" in error for error in result["errors"])
