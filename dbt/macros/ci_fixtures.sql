{% macro apply_ci_fixtures() %}
    {% set use_seeds = env_var('CI_USE_SEEDS', '') | lower %}
    {% if use_seeds in ['1', 'true', 'yes', 'on'] %}
        ...
        {% for fixture in fixtures %}
            ...
            {% if seed_relation %}
                ...
                {% do log('Created CI fixture view for ' ~ target_relation, info=True) %}
            {% else %}
                {% do log('Skipping CI fixture view for ' ~ fixture.identifier ~ ' because seed relation was not found.', info=True) %}
            {% endif %}
        {% endfor %}
    {% else %}
        {% do log('CI_USE_SEEDS unset; skipping CI fixture view creation.', info=False) %}
    {% endif %}
{% endmacro %}
