...
filters as (
    select
        to_jsonb(coalesce(array_agg(distinct source_platform) filter (where source_platform is not null), array[]::text[])) as source_platforms,
        to_jsonb(coalesce(array_agg(distinct ad_account_id) filter (where ad_account_id is not null), array[]::text[])) as ad_account_ids
    from scoped_campaigns
),
campaigns_json as (
    select coalesce(
    ...
    select
        jsonb_build_object(
            ...
            'filters', jsonb_build_object(
                'sourcePlatforms', coalesce(filters.source_platforms, '[]'::jsonb),
                'adAccountIds', coalesce(filters.ad_account_ids, '[]'::jsonb)
            )
        ) as payload
    from generated
    cross join summary
...
violations as (
    ...
    from payload
    where jsonb_typeof(payload->'summary') != 'object'
    ...
    from payload
    where jsonb_typeof(payload->'generatedAt') != 'string'

    union all

    select 'filters_missing_object' as error, payload
    from payload
    where jsonb_typeof(payload->'filters') != 'object'
    ...
    from payload
    where jsonb_typeof((payload->'filters')->'sourcePlatforms') != 'array'

    union all

    select 'filters_accounts_not_array' as error, payload
    from payload
    where jsonb_typeof((payload->'filters')->'adAccountIds') != 'array'
    ...
    from payload
    where jsonb_typeof(payload->'campaigns') != 'array'
    ...
    from payload
    where jsonb_typeof(payload->'parishes') != 'array'
