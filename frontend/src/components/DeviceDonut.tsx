...
import type { DeviceBreakdown } from '../state/useDashboardStore';
import { chartPalette } from '../styles/chartTheme';
import { formatNumber } from '../lib/format';

const DEVICE_COLORS: Record<string, string> = {
  mobile_app: chartPalette[0],
  mobile_web: chartPalette[1],
  desktop: chartPalette[2],
};
...
const DeviceDonut = ({ data, metric = 'impressions' }: DeviceDonutProps) => {
  const chartData = useMemo(
    () =>
      data.map((row) => ({
        name: DEVICE_LABELS[row.device.toLowerCase()] ?? row.device,
        value: Number(row[metric] ?? 0),
        device: row.device.toLowerCase(),
      })),
    [data, metric],
  );
  ...
  return (
    <ResponsiveContainer width="100%" height={260}>
      <PieChart>
        ...
        >
          {chartData.map((entry) => (
            <Cell key={entry.device} fill={DEVICE_COLORS[entry.device] ?? chartPalette[3]} />
          ))}
        </PieComponent>
        <TooltipComponent
          contentStyle={{
