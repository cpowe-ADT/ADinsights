...
from integrations.google_ads.query_reference import ingest_query_reference_file


class Command(BaseCommand):
    help = (
        "Parse Google Ads queryable resource reference text and persist it as a "
    ...
    def add_arguments(self, parser):  # noqa: ANN001 - Django command signature
        parser.add_argument(
            "--input",
            required=True,
            help="Path to raw Google Ads queryable resources reference text.",
        )
        parser.add_argument(
            "--output",
            required=False,
    ...
    def handle(self, *args, **options):  # noqa: ANN002, ANN003 - Django command signature
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Imported Google Ads query reference ({version}) with "
                f"{payload.get('resource_count', 0)} resource types."
            )
        )
