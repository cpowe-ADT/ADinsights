...
class MetaOAuthExchangeView(APIView):
    ...
    def post(self, request, *args, **kwargs):  # noqa: ANN001 - DRF signature
        ...
        try:
            ...
            logger.info(
                "meta.oauth.exchange.invalid_payload",
                extra={
                    "tenant_id": str(request.user.tenant_id),
                    "error": str(exc),
                },
            ...
            logger.warning(
                "meta.oauth.exchange.misconfigured",
                extra={
                    "tenant_id": str(request.user.tenant_id),
                    "error": str(exc),
                },
            ...
            return Response(
                {"detail": "Meta OAuth is not configured for this environment."},
                status=status.HTTP_503_SERVICE_UNAVAILABLE,
            )
        except MetaGraphClientError as exc:
            logger.warning(
                "meta.oauth.exchange.graph_error",
                extra={
                    "tenant_id": str(request.user.tenant_id),
                    "error": str(exc),
                },
            ...
            return Response(
                {"detail": "Meta OAuth exchange failed. Retry the connect flow."},
    ...
    def post(self, request, *args, **kwargs):  # noqa: ANN001 - DRF signature
        ...
        try:
            ...
            return Response(
                {
                    "detail": (
                        "Stored Meta Page connection could not be validated. "
                        "Reconnect Meta with Facebook."
                    ),
            ...
            return Response(
                {
                    "detail": "Unable to recover Meta marketing access right now.",
