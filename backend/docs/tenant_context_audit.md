...
| Background jobs | `backend/core/crypto/dek_manager.py`, `backend/integrations/tasks.py`, `backend/integrations/airbyte/service.py` | Loops now invoke `tenant_context(...)` per tenant (Aug 2024); global sweeps use `tenant_context(None)`. | Keep coverage in `backend/tests/test_tasks.py` up to date and add assertions for Airbyte webhook flows. |
...
- [ ] Pytest coverage proves tenant A cannot read tenant B via APIs, Celery tasks, or adapters. *(see TODO: extend Celery leak tests)*
