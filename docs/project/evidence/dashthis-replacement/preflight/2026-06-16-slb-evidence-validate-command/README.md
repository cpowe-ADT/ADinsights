# Preflight Packet For SLB Evidence Validate Command
...
backend/.venv/bin/python docs/ops/skills/adinsights-release-readiness/scripts/run_preflight_skillchain.py \
  --prompt "Assess SLB evidence validation command for DashThis cancellation readiness" \
  --changed-files-from-git \
  --output-dir docs/project/evidence/dashthis-replacement/preflight/2026-06-16-slb-evidence-validate-command \
  --format markdown
```
...
`slb_report_evidence_validate` command and related evidence docs. The command improves pre-G10 and
pre-G11 evidence artifact checks, but it does not clear G0 or prove cancellation-review readiness.
