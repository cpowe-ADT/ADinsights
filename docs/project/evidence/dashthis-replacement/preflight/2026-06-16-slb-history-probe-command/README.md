# Preflight Packet For SLB History Probe Command
...
backend/.venv/bin/python docs/ops/skills/adinsights-release-readiness/scripts/run_preflight_skillchain.py \
  --prompt "Assess SLB retained-history probe command for DashThis cancellation readiness" \
  --changed-files-from-git \
  --output-dir docs/project/evidence/dashthis-replacement/preflight/2026-06-16-slb-history-probe-command \
  --format markdown
```
...
`slb_report_history_probe` command and related G2/G3 evidence docs. The command improves monthly
and retained-history evidence collection, but it does not prove fixed-target coverage or clear
Raj/Mira architecture review.
