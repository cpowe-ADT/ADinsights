...
Date: 2026-06-25; updated 2026-06-26 for catalog availability states
...
| Manual paid CSV import       | `backend/manage.py import_meta_paid_csv` → `RawPerformanceRecord(source="meta")` + `Campaign` metadata                                                                                                                                                                       | Operator-only fallback for approved daily Meta Ads UI/export rows when the selected ad account cannot be API-backfilled. No live Meta call; rejects multi-day aggregate rows so coverage is not overstated.                                  |
...
| Manual organic CSV import    | `backend/manage.py import_meta_organic_csv` → `MetaInsightPoint` + `MetaPostInsightPoint`                                                                                                                                                                                    | Operator-only fallback for approved Meta UI/export aggregate reach/impression values when Graph `/insights` is gated. No live Meta call, no `read_insights`, no user-level identity.                                                         |
...
| Metric validity              | `meta_page_insights.insights_discovery.validate_metrics` → `MetaMetricSupportStatus`; report catalog `availability_state`                                                                                                                                                    | Low-level discovery still marks `supported=True` on any non-error response (incl. 200-empty). The governed report catalog now separates `available`, `callable_no_data`, `permission_gated`, and `unsupported` for builder/report selection. |
...
| Report render                | `analytics.reporting_preview` (reads stored tables only) + `analytics.reporting_source_health` (coverage + truthful messaging + redacted remediation command templates)                                                                                                      | No live Meta calls during preview/export. Source-health now points operators to selected-account paid backfill, manual organic CSV import, Page/Post backfill, or Content Ops snapshot refresh without exposing tenant/Page/account IDs.     |
...
4. **Misleading low-level "supported" status.** `validate_metrics` / `_mark_metric_support` still set
   `supported=True` for metrics that return 200-with-empty-data (read_insights-gated). The reporting
   catalog now exposes higher-level metric `availability_state`, but the discovery table itself still
   needs the same state model if operators use it directly.
