...
- **SLO:** Backend release smoke guard remains green for 99% of `main` runs.
  - **SLI source:** The `scope=release_smoke,metric=status` row in `ci-metrics.csv` and the `release_smoke.ok` field in `backend-ci-summary.json`.
