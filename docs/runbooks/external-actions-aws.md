...
Operator command pack: `docs/runbooks/external-actions-execution-checklist.md`.
...
| `P2-C1` | Connector OAuth app production approvals | Google Cloud Console + Meta App Dashboard | Mei + Maya + Nina | 1) Publish Google OAuth consent screen for production scopes (`adwords`, `analytics.readonly`, `webmasters.readonly`). 2) Configure authorized redirect URI(s) for backend-driven OAuth callback flow. 3) Complete Meta app review/permissions for required page scopes. 4) Confirm production app mode and allowed domains. | Completed `connector-oauth-approvals-<date>-est.md` with app IDs, approved scopes, reviewer notes, and screenshot evidence. | Google + Meta OAuth apps are approved and production-ready for tenant onboarding. |
